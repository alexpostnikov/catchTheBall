...
class ur10svh(ur10SvhBase):
    ...
    def __init__(self, config, resource_directory=ANYMAL_RESOURCE_DIRECTORY,  video_folder=None):
    ...
    def step(self, action):
        ...
        for _ in range(loop_count):
            ...
            if self.visualizable and (self.visualization_counter % vis_decimation == 0):
                ...
                raisim.gui.reward_logger.log("obsEndef[0]", self.obsEndef[0])
                raisim.gui.reward_logger.log("obsEndef[1]", self.obsEndef[1])
                raisim.gui.reward_logger.log("obsEndef[2]", self.obsEndef[2])
                vis = raisim.OgreVis.get()
    ...
    def is_terminal_state(self):
        ...
        if (self.step_number >= self.max_step) or (self.getBallPose()[2] < 0.5):
            ...
            if (self.terminal_counter % self.config["environment"]["render_every_n_resets"] == 0) and (self.visualize_this_step):
                self.terminal_counter = 1
                self.visualizable = True
        return self.done
    ...
    def reset_ball(self):
        ...
        if self.config["environment"]["curruclum"]["use_curruclum"]:
            ...
            self.disp = self.disp * np.random.rand(3)
